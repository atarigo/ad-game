...
export async function initApp(el: HTMLElement, bg = COLORS.bg) {
	...
	return { app, cleanup: () => { ro.disconnect(); app.destroy(true); } };
}
...
export function initPhysics(app: Application, gravity = { x: 0, y: 1, scale: 0.001 }) {
...
export function drawCircle(x: number, y: number, r: number, color: number): Graphics {
	const g = new Graphics();
	g.circle(0, 0, r);
	g.fill(color);
	g.x = x;
